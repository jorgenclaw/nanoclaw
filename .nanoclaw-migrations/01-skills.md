...
## Corrections found during the 2026-07-22 upgrade

The audit above missed 3 skills that exist on SOURCE but not on bare `upstream/main`:
- `claw` — genuinely custom (upstream has no CLI-install skill of this name), wholesale-ported.
- `add-parallel` — active skill (Parallel AI integration), wholesale-ported.
- `convert-to-apple-container` — likely a leftover from before upstream moved Apple Container support to a skill branch; harmless to keep (just a skill file, no runtime cost), ported for optionality even though this install runs Docker on Linux, not Apple Container.

Also: the `update` skill this guide said to wholesale-port turned out to be **broken** — its `fetch-upstream.sh` depends on a `skills-engine/constants.ts` file that doesn't exist anywhere in this codebase (confirmed by attempting to run it during the 2026-07-22 `/update` invocation, before switching to `/update-nanoclaw`). **Not ported** — `update-nanoclaw` and `migrate-nanoclaw`, which upstream ships natively, cover the same need and actually work.
