...
async function runQuery(
  ...
  resumeAt?: string,
): Promise<{ newSessionId?: string; lastAssistantUuid?: string; closedDuringQuery: boolean; aborted: boolean; inputTokens: number; outputTokens: number }> {
  ...
  clearTimeout(idleTimer);
  ipcPolling = false;
  log(`Query done. Messages: ${messageCount}, results: ${resultCount}, tokens: ${inputTokens}in/${outputTokens}out, lastAssistantUuid: ${lastAssistantUuid || 'none'}, closedDuringQuery: ${closedDuringQuery}, aborted: ${aborted}`);
  return { newSessionId, lastAssistantUuid, closedDuringQuery, aborted, inputTokens, outputTokens };
}
