...
const MCP_SERVER_PORT = parseInt(process.env.MCP_SERVER_PORT || mcpEnv.MCP_SERVER_PORT || '3002', 10);
const MCP_SERVER_NAME = process.env.MCP_SERVER_NAME || mcpEnv.MCP_SERVER_NAME || 'Jorgenclaw Sovereign MCP';
const TOOL_PRICE_SIGN = parseInt(process.env.TOOL_PRICE_SIGN || mcpEnv.TOOL_PRICE_SIGN || '2', 10);
const TOOL_PRICE_PUBLISH = parseInt(process.env.TOOL_PRICE_PUBLISH || mcpEnv.TOOL_PRICE_PUBLISH || '3', 10);
const TOOL_PRICE_POST_NOTE = parseInt(process.env.TOOL_PRICE_POST_NOTE || mcpEnv.TOOL_PRICE_POST_NOTE || '4', 10);
const TOOL_PRICE_FETCH_PROFILE = parseInt(process.env.TOOL_PRICE_FETCH_PROFILE || mcpEnv.TOOL_PRICE_FETCH_PROFILE || '1', 10);
const TOOL_PRICE_ZAP = parseInt(process.env.TOOL_PRICE_ZAP || mcpEnv.TOOL_PRICE_ZAP || '5', 10);
const TOOL_PRICE_GET_NOTES = parseInt(process.env.TOOL_PRICE_GET_NOTES || mcpEnv.TOOL_PRICE_GET_NOTES || '2', 10);
const TOOL_PRICE_CREATE_INVOICE = parseInt(process.env.TOOL_PRICE_CREATE_INVOICE || mcpEnv.TOOL_PRICE_CREATE_INVOICE || '1', 10);
const TOOL_PRICE_ACTION_RECEIPT = parseInt(process.env.TOOL_PRICE_ACTION_RECEIPT || mcpEnv.TOOL_PRICE_ACTION_RECEIPT || '3', 10);
const TOOL_PRICE_VERIFY_RECEIPT = parseInt(process.env.TOOL_PRICE_VERIFY_RECEIPT || mcpEnv.TOOL_PRICE_VERIFY_RECEIPT || '1', 10);
...
const ATTESTATION_RELAYS = ['wss://relay.damus.io', 'wss://nos.lol', 'wss://relay.nostr.band', 'wss://relay.primal.net'];
...
function daemonRequest(payload: object): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    ...
    sock.on('data', (chunk: Buffer) => { data += chunk; });
    sock.on('end', () => {
      try { resolve(JSON.parse(data)); }
      catch { reject(new Error(`Bad response from signer: ${data}`)); }
    });
    sock.on('error', (err) => reject(new Error(`Cannot connect to signing daemon: ${err.message}`)));
  });
}

// --- Invoice helper ---

function generateInvoice(amountSats: number, description: string): string | null {
  try {
    const result = execSync(
      `${process.execPath} ${walletScript} invoice ${amountSats} "${description.replace(/"/g, '\\"')}"`,
      { encoding: 'utf8', timeout: 15000, env: { ...process.env, NWC_CONFIG: NWC_CONFIG_PATH, NWC_SPENDING: NWC_SPENDING_PATH } },
    );
...
async function publishToRelays(event: Record<string, unknown>, relays: string[]): Promise<string[]> {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        ws.on('message', (msg: Buffer) => {
          ...
          if (parsed[0] === 'OK' && parsed[1] === (event as { id?: string }).id) {
  ...
  const promises = QUERY_RELAYS.map(async (url) => {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        ws.on('open', () => ws.send(JSON.stringify(['REQ', subId, { ...filter, limit }])));
        ws.on('message', (msg: Buffer) => {
          const parsed = JSON.parse(msg.toString());
          ...
          if (parsed[0] === 'EOSE') { clearTimeout(timeout); ws.close(); resolve(); }
        });
...
function walletExec(command: string): string {
  return execSync(
    `${process.execPath} ${walletScript} ${command}`,
    { encoding: 'utf8', timeout: 30000, env: { ...process.env, NWC_CONFIG: NWC_CONFIG_PATH, NWC_SPENDING: NWC_SPENDING_PATH, NOSTR_SIGNER_SOCKET } },
  ).trim();
}
...
function registerTools(server: McpServer): void {
  ...
  server.tool(
    ...
    `Sign a Nostr event using a sovereign signing daemon. Price: ${TOOL_PRICE_SIGN} sats.`,
    {
      ...
      payment_preimage: z.string().optional().describe('Lightning payment preimage (64 hex chars) — provide after paying the invoice'),
    },
    async (args) => {
      ...
      if (!verifyPreimage(args.payment_preimage)) return errorResponse('Invalid payment preimage');

      try {
        const result = await daemonRequest({
          method: 'sign_event',
          params: { kind: args.kind, content: args.content, tags: args.tags || [], created_at: args.created_at },
        });
        if (result.error) return errorResponse(String(result.error));
        logger.info({ tool: 'nostr_sign_event', kind: args.kind }, 'Paid tool call executed');
        return { content: [{ type: 'text' as const, text: JSON.stringify({ signed_event: result.event }) }] };
      } catch (err) {
  ...
  server.tool(
    ...
    `Sign and publish a Nostr event to relays. Price: ${TOOL_PRICE_PUBLISH} sats.`,
    {
      ...
      tags: z.array(z.array(z.string())).optional().describe('Event tags'),
      created_at: z.number().optional().describe('Unix timestamp'),
      relays: z.array(z.string()).optional().describe('Relay URLs (defaults to popular relays)'),
      payment_preimage: z.string().optional().describe('Lightning payment preimage (64 hex chars) — provide after paying the invoice'),
    },
    async (args) => {
      if (!args.payment_preimage) {
      ...
      try {
        ...
        if (publishedTo.length === 0) return errorResponse('Failed to publish to any relay');

        logger.info({ tool: 'nostr_publish_event', kind: args.kind, relays: publishedTo.length }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ event_id: signedEvent.id, published_to: publishedTo, signed_event: signedEvent }),
          }],
        };
      } catch (err) {
        return errorResponse(err instanceof Error ? err.message : String(err));
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        ...
        logger.info({ tool: 'nostr_post_note', relays: publishedTo.length }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ event_id: signedEvent.id, published_to: publishedTo }),
          }],
        };
      } catch (err) {
        return errorResponse(err instanceof Error ? err.message : String(err));
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        ...
        logger.info({ tool: 'nostr_fetch_profile', pubkey: hex.slice(0, 16) }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({
  ...
  server.tool(
    ...
    `Zap a Nostr user via Lightning. Price: ${TOOL_PRICE_ZAP} sats (tool fee) + zap amount. Max 5000 sats.`,
    {
      ...
      amount_sats: z.number().min(1).max(5000).describe('Zap amount in sats (max 5000)'),
    ...
    async (args) => {
      ...
      try {
        ...
        logger.info({ tool: 'nostr_zap', amount: args.amount_sats }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ paid: true, preimage: parsed.preimage, amount_sats: args.amount_sats }),
          }],
        };
      } catch (err) {
        return errorResponse(err instanceof Error ? err.message : String(err));
  ...
  server.tool(
    ...
    async (args) => {
      ...
      if (!verifyPreimage(args.payment_preimage)) return errorResponse('Invalid payment preimage');
      if (!args.author && !args.hashtag) return errorResponse('Provide author or hashtag (or both)');
      ...
      try {
        ...
        logger.info({ tool: 'nostr_get_notes', count: notes.length }, 'Paid tool call executed');
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        const desc = (args.description || 'NanoClaw invoice').replace(/"/g, '\\"');
        const result = walletExec(`invoice ${args.amount_sats} "${desc}"`);
        const parsed = JSON.parse(result);

        if (!parsed.invoice) return errorResponse('Failed to create invoice');

        logger.info({ tool: 'lightning_create_invoice', amount: args.amount_sats }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({ bolt11: parsed.invoice, amount_sats: args.amount_sats, description: args.description }),
          }],
        };
      } catch (err) {
        return errorResponse(err instanceof Error ? err.message : String(err));
  ...
  server.tool(
    ...
    {
      agent_id: z.string().describe('Agent identifier (e.g. "myagent@example.com")'),
      action: z.string().describe('Action identifier (e.g. "posted_to_moltbook")'),
      summary: z.string().describe('Human-readable description of the action'),
      metadata: z.record(z.string(), z.string()).optional().describe('Optional key-value pairs for additional context'),
      payment_preimage: z.string().optional().describe('Lightning payment preimage (64 hex chars)'),
    },
    async (args) => {
      if (!args.payment_preimage) {
      ...
      try {
        ...
        const publishedTo = await publishToRelays(signedEvent, ATTESTATION_RELAYS);
        if (publishedTo.length === 0) return errorResponse('Failed to publish receipt to any relay');

        const eventId = signedEvent.id as string;
        logger.info({ tool: 'create_action_receipt', agent: args.agent_id, action: args.action, relays: publishedTo.length }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({
              ...
              signed_at: new Date((signedEvent.created_at as number) * 1000).toISOString(),
              pubkey: OUR_PUBKEY,
            }),
          }],
        };
  ...
  server.tool(
    ...
    {
      event_id: z.string().describe('Nostr event ID of the action receipt to verify'),
      payment_preimage: z.string().optional().describe('Lightning payment preimage (64 hex chars)'),
    },
    async (args) => {
      if (!args.payment_preimage) {
      ...
      try {
        ...
        if (events.length === 0) return errorResponse('Event not found on relays');
        ...
        logger.info({ tool: 'verify_receipt', eventId: args.event_id, valid }, 'Paid tool call executed');
        return {
          content: [{
            type: 'text' as const,
            text: JSON.stringify({
              ...
              summary: (ev.content as string || '').split('\nSummary: ')[1] || null,
              created_at: new Date((ev.created_at as number) * 1000).toISOString(),
...
export async function startMcpServer(): Promise<void> {
  ...
  const httpServer = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url === '/health' && req.method === 'GET') {
      ...
      res.end(JSON.stringify({ status: 'ok', name: MCP_SERVER_NAME, tools: ['nostr_sign_event', 'nostr_publish_event', 'nostr_post_note', 'nostr_fetch_profile', 'nostr_zap', 'nostr_get_notes', 'lightning_create_invoice', 'create_action_receipt', 'verify_receipt'] }));
      return;
    }
    ...
    if (url === '/sse' && req.method === 'GET') {
      ...
      const sessionServer = new McpServer({ name: MCP_SERVER_NAME, version: '1.0.0' });
    ...
    res.end(JSON.stringify({ error: 'Not found. Use /sse for MCP or /health for status.' }));
  });

  httpServer.listen(MCP_SERVER_PORT, '0.0.0.0', () => {
    logger.info({ port: MCP_SERVER_PORT, name: MCP_SERVER_NAME }, 'Paid MCP server started');
  });
}
