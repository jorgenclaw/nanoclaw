...
// Pino-style shim: logger.info({data}, 'msg') → log.info('msg', {data})
const logger = {
...
async function isFreeTierCall(nip05?: string): Promise<boolean> {
  ...
  logger.info({ nip05, count: count + 1, limit: FREE_TIER_LIMIT }, 'Free tier call');
  return true;
}
...
const TOOL_PRICE_SIGN = parseInt(process.env.TOOL_PRICE_SIGN || mcpEnv.TOOL_PRICE_SIGN || '21', 10);
const TOOL_PRICE_PUBLISH = parseInt(process.env.TOOL_PRICE_PUBLISH || mcpEnv.TOOL_PRICE_PUBLISH || '21', 10);
const TOOL_PRICE_POST_NOTE = parseInt(process.env.TOOL_PRICE_POST_NOTE || mcpEnv.TOOL_PRICE_POST_NOTE || '21', 10);
const TOOL_PRICE_FETCH_PROFILE = parseInt(
  process.env.TOOL_PRICE_FETCH_PROFILE || mcpEnv.TOOL_PRICE_FETCH_PROFILE || '5',
  10,
);
const TOOL_PRICE_ZAP = parseInt(process.env.TOOL_PRICE_ZAP || mcpEnv.TOOL_PRICE_ZAP || '50', 10);
const TOOL_PRICE_GET_NOTES = parseInt(process.env.TOOL_PRICE_GET_NOTES || mcpEnv.TOOL_PRICE_GET_NOTES || '5', 10);
const TOOL_PRICE_CREATE_INVOICE = parseInt(
  process.env.TOOL_PRICE_CREATE_INVOICE || mcpEnv.TOOL_PRICE_CREATE_INVOICE || '5',
  10,
);
const TOOL_PRICE_ACTION_RECEIPT = parseInt(
  process.env.TOOL_PRICE_ACTION_RECEIPT || mcpEnv.TOOL_PRICE_ACTION_RECEIPT || '21',
  10,
);
const TOOL_PRICE_VERIFY_RECEIPT = parseInt(
  process.env.TOOL_PRICE_VERIFY_RECEIPT || mcpEnv.TOOL_PRICE_VERIFY_RECEIPT || '5',
  10,
);
...
function registerTools(server: McpServer): void {
  ...
  server.tool(
    ...
    async (args) => {
      ...
      if (!free && !verifyPreimage(args.payment_preimage!)) return errorResponse('Invalid payment preimage');
