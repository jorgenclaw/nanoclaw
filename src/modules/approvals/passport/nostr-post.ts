...
registerDeliveryAction(
  NOSTR_POST_ACTION,
  (content, session, inDb) => handleNostrPost(content, session, inDb, getAuthorizationService(), clawstrPostExecutor()),
  unguarded(
    'gated by the Passport signed-authorization handshake (host executes the post itself only ' +
      'after a verified APPROVE) — see .nanoclaw-migrations/07-passport-approval-gates.md',
  ),
);
