...
// Lists — create/edit are unguarded, same trust level as x_follow: they
...
const LISTS_UNGUARDED_REASON =
  'X-integration list tool — create/rename/re-privacy a list or add/remove members; reversible by the agent, same trust level as x_follow';
registerDeliveryAction(
  'x_read_my_lists',
  makeXHandler({ action: 'x_read_my_lists', scriptName: 'read-my-lists', buildArgs: () => ({}) }),
  unguarded(LISTS_UNGUARDED_REASON),
);
registerDeliveryAction(
  'x_read_list_members',
  makeXHandler({
